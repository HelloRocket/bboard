...
in_folder = input('Which BitsBoard do you want?')

# detect the current working directory and print it
path = os.getcwd()  
print ("The current working directory is %s" % path)  

local_path = path+"/"+in_folder

try:  
    os.mkdir(local_path)
except OSError:  
    print ("Creation of the directory %s failed" % local_path)
else:  
    print ("Successfully created the directory %s " % local_path)

db_path = '/Bitsboard/'+in_folder+"/"
local_file_path = local_path+"/"


dbx = dropbox.Dropbox('QmvUpb2ZlWQAAAAAAAAYYqmDrQajPJanG9YgFPFWqhGa76a7k8XEYha_Wcq4tOh5')
#dbx = dropbox.Dropbox('MqEtDgWwuh0AAAAAAAB9cW0riBiwHhnQBiDKeS777ghVAoVAnnWJIFFXmkGXxJMn')

for entry in dbx.files_list_folder(db_path).entries:
    print(entry.name)
    str_entry_name = str(entry.name)
    
    entry_to_download = db_path+str_entry_name
    print(entry_to_download)
    local_file_name = local_file_path + str_entry_name
    print(local_file_name)
    dbx.files_download_to_file(local_file_name, entry_to_download)


'''fileName is the plist to import'''
folder_location = in_folder+'/'
fileName = folder_location +'Bitsboard.plist'
...
class Photorocket (ui.View):
    ...
    def playTouch(self, importList):    
        ...
        for i in range(3):
            ...
            picname =  folder_location + targetPic.lower()
        ...
        mp3sound=folder_location+ winbit.lower()+'.mp3'
        cafsound=folder_location+ winbit.lower()+'.caf'
        sound.play_effect(mp3sound)
        sound.play_effect(cafsound)
